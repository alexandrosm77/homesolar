...
def dashboard_snapshot(session: Session, now: datetime | None = None) -> DashboardSnapshot:
    ...
    live_polls_by_inverter = latest_poll_events(session, inverter_ids, kind="live")
    ...
    for inverter in inverters:
        ...
        last_live_poll = live_polls_by_inverter.get(inverter.id)
        ...
        health = telemetry_health(inverter, latest, last_poll, alarm, now, last_live_poll)
...
def telemetry_health(
    ...
    stale_deadline = stale_after_seconds(inverter.live_poll_seconds)
    live_poll_age_seconds = (
        _age_seconds(last_live_poll.started_at, now) if last_live_poll else None
    )
    is_stale = bool(
        inverter.enabled
        and live_poll_age_seconds is not None
        and live_poll_age_seconds > stale_deadline
    )
    is_online = bool(
        latest
        and last_poll
